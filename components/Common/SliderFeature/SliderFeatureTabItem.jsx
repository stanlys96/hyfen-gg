...
const SliderFeatureTabItem = ({
  ...
  return (
    ...
    >
      <h2 className={`text-2xl font-bold transition-all mb-3 hover:text-white ${active ? "text-white" : "text-gray-200"}`}>{t(title)}</h2>
      <Collapse isOpen={active}>
        <p className=" text-sm md:text-xl text-gray-200">{t(description)}</p>
      </Collapse>
    </div>
  )
}
